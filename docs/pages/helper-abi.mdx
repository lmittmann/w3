...
Function signatures are defined using Solidity syntax. Arguments and return variables can optionally be named. While naming is optional, it is recommended for more complex functions or tuple variables.
...
```
```go
funcBalanceOf := w3.MustNewFunc("balanceOf(address who)", "uint256 amount")
```
...
|:-------------------------|:---------------------|
| `bool`                   | `bool`               |
| `uint`/`uint8`…`uint256` | `*big.Int`           |
| `int`/`int8`…`int256`    | `*big.Int`           |
| `bytes`                  | `[]byte`             |
| `bytes1`…`bytes32`       | `[1]byte`…`[32]byte` |
| `address`/`bytes20 `     | `common.Address`     |
| `bytes32`                | `common.Hash`        |
