...
<DocLink title="w3vm.VM" /> is a super simple to use Ethereum Virtual Machine (EVM), built on top of `go-ethereum/core/vm.EVM`. It supports **tracing**, **state forking** via RPC, and can be used for simulation, debugging EVM execution, or testing Smart Contracts.
